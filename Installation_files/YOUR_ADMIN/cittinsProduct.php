...
$cittins = new cittinsProduct();
$productType = (isset($_POST['product_type']) ? $_POST['product_type'] : (isset($_GET['pID']) ? $cittins->getProductType($_GET['pID']) : 1));
...
foreach ($productTypeFields as $productFields) {
    ...
    if ($productFields['tab_id'] != '0') {
        require PRODUCT_FIELDS_INCLUDES_SQL_FOLDER . $productFields['field_name'] . '.php';
    }
}
...
$width = '';
$height = '';
list($imageWidth, $imageHeight) = getimagesize(DIR_FS_CATALOG_IMAGES . $productInformation->products_image['value']);
...
$mediumHeight = (int)MEDIUM_IMAGE_HEIGHT;
if ($imageHeight > $mediumHeight) {
    $height = $mediumHeight;
} else {
    $height = $imageHeight;
}
        ...
        <div class="container-fluid">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form name="productInfo" method="post" enctype="multipart/form-data" id="productInfo" class="form-horizontal">
                        <?php
                                    ...
                                    foreach ($productInformation as $key => $infoField) {
                                        ...
                                        if ($infoField['tabId'] == $tab['id']) {
                                            ...
                                            <div class="form-group">
                                                <?php require_once PRODUCT_FIELDS_LANGUAGE_DEFINES . $key . '.php'; ?>
                                                <?php require INCLUDES_HTML_OUTPUT_FOLDER . $key . '.php'; ?>
                                            </div>
                                            <?php
                                        }
                                    }
                            ...
                            if (isset($extraTabsFiles) && $extraTabsFiles != '') {
                                ...
                                foreach ($extraTabsFiles as $tabContent) {
                                    if (strpos($tabContent, $tabContentsNeedle) !== false) {
                                        $tabContentLanguage = substr($tabContent, 0, stripos($tabContent, '/'));
                                        ?>
                                        <div id="productTabs<?php echo $tabSortContent; ?>" class="tab-pane fade">
                                            <?php require_once EXTRA_TABS_LANGUAGE_DEFINES . $tabContentLanguage . '.php'; ?>
                                            <?php require DIR_WS_MODULES . 'extra_tabs/' . $tabContent; ?>
                                        </div>
                                        <?php
                                        $tabSortContent++;
                                    }
