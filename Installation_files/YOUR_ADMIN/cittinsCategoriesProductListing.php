...
if (zen_not_null($action)) {
  switch ($action) {
    case 'set_editor':
      ...
      $action = '';
      zen_redirect(zen_href_link(FILENAME_CITTINS_CATEGORIES_PRODUCT_LISTING, 'cPath=' . $_GET['cPath'] . (!empty($productId) ? '&pID=' . $productId : '') . ($currentPage != 0 ? '&page=' . $currentPage : '')));
      break;
      ...
      <div class="panel panel-default">
        ...
        <div class="panel-body">
          ...
          <div class="col-md-4">
            ...
            if ($search_result) {
              ...
              <div class="form-group">
                ...
                <div class="col-sm-6 col-md-8">
                  ...
                  <a href="<?php echo zen_href_link(FILENAME_CITTINS_CATEGORIES_PRODUCT_LISTING); ?>" class="btn btn-default" role="button"><?php echo IMAGE_RESET; ?></a>
                </div>
              </div>
              <?php
            }
            echo '</form>';
            ?>
            <?php echo zen_draw_form('goto', FILENAME_CITTINS_CATEGORIES_PRODUCT_LISTING, '', 'get', 'class="form-horizontal"'); ?>
            <div class="form-group">
          ...
          <?php //echo zen_draw_form('listing', FILENAME_CITTINS_CATEGORIES_PRODUCT_LISTING, zen_get_all_get_params(), 'post', 'class="form-horizontal"'); ?>
          <form name="multiSelectListingActions" class="form-horizontal" method="post" id="multiSelectListingActionsForm">
            <table class="table table-striped" id="categoriesProductListing">
              <thead>
                <tr valign="middle">
                  <th><?php echo zen_draw_checkbox_field('select_all', 'select_all', false, '', 'id="select_all"'); ?></th>
              ...
              <tbody>
                <tr>
                  ...
                  <td class="ColumnPrice text-right hidden-sm hidden-xs">&nbsp;</td>
                  <td class="ColumnQuantity text-right hidden-sm hidden-xs">&nbsp;</td>
                  <td class="ColumnStatus text-right hidden-sm hidden-xs text-nowrap">&nbsp;</td>
                  <td class="ColumnSort text-right hidden-sm hidden-xs">&nbsp;</td>
                  <td>&nbsp;</td>
                </tr>
                <?php
                ...
                foreach ($categories as $category) {
                  ...
                  <tr id="cID_<?php echo $category['categories_id']; ?>">
                    ...
                    <td class="text-right">
                      <div class="btn-group">
                        <a href="<?php echo zen_href_link(FILENAME_CITTINS_CATEGORIES, 'cPath=' . $cPath . '&cID=' . $category['categories_id'] . ($searchWords != '' ? '&search=' . $searchWords : '')); ?>" title="<?php echo TEXT_LISTING_EDIT; ?>" class="btn btn-sm btn-info" role="button">
...
// Get categories_id for product if search
                  ...
                  <tr id="pID_<?php echo $product['products_id']; ?>">
                    ...
                    <td class="ColumnName"><a href="<?php echo zen_catalog_href_link($type_handler . '_info', (!empty($cPath) ? 'cPath=' . $cPath . '&' : '') . 'products_id=' . $product['products_id'] . '&language=' . $_SESSION['languages_code'] . '&product_type=' . $product['products_type']); ?>" target="_blank"><?php echo zen_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW); ?></a>&nbsp;<a href="<?php echo zen_href_link(FILENAME_CITTINS_PRODUCT, (!empty($cPath) ? 'cPath=' . $cPath . '&' : '') . 'product_type=' . $product['products_type'] . '&pID=' . $product['products_id'] . '&action=new_product' . ($searchWords != '' ? '&search=' . $searchWords : '')); ?>" title="<?php echo TEXT_LISTING_EDIT; ?>"><?php echo $product['products_name']; ?></a></td>
                    <td class="ColumnImage">
                    ...
                    <td class="text-right">
                      <div class="btn-group">
                        <a href="<?php echo zen_href_link(FILENAME_CITTINS_PRODUCT, (!empty($cPath) ? 'cPath=' . $cPath . '&' : '') . 'product_type=' . $product['products_type'] . '&pID=' . $product['products_id'] . '&action=new_product' . ($searchWords != '' ? '&search=' . $searchWords : '')); ?>" title="<?php echo TEXT_LISTING_EDIT; ?>" class="btn btn-sm btn-info" role="button">
                          <i class="fa fa-pencil fa-lg" aria-hidden="true"></i>
                        </a>
                        ...
                        <?php if (zen_has_product_attributes($product['products_id'], 'false')) { ?>
                          <a href="<?php echo zen_href_link(FILENAME_CITTINS_CATEGORIES_PRODUCT_LISTING, 'cPath=' . $cPath . '&pID=' . $product['products_id'] . '&action=attribute_features' . ($currentPage != 0 ? '&page=' . $currentPage : '')); ?>" title="<?php echo TEXT_LISTING_ATTRIBUTES; ?>" class="btn btn-sm btn-info" role="button">
          ...
          <div class="row mt5">
            <div class="col-sm-6 text-right">
              ...
              if ($zc_skip_products == false) {
                ...
                ?>
                <?php echo zen_draw_form('newproduct', FILENAME_CITTINS_PRODUCT, 'action=new_product', 'post', 'class="form-horizontal"'); ?>
                <div class="input-group">
                  <div class="input-group-btn"><button type="submit" class="btn btn-primary"><?php echo IMAGE_NEW_PRODUCT; ?></button></div>
                  <?php echo zen_draw_pull_down_menu('product_type', $product_restrict_types_array, '', 'class="form-control"'); ?>
                </div>
                <?php
                echo zen_hide_session_id();
    ...
    foreach (glob(DIR_WS_MODALS . 'cittins/categoriesProductListing/*.php') as $filename) {
      include $filename;
    }
